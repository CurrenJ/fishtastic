...
|---|---|---|---|
| `port/1.21.1` | **`7839f271`** | 2026-09-25 | Pass 2 written; rebased onto the S5/S6/S6c seams. A1–A5 done; **G1 passed 2026-09-25** (all 11 self-test scenes green on both loaders, Iris live on both, `:common:test` 78/78, fishsim byte-identical to `26.1.2`). **A6.1 in progress**: one shared `FishtasticGameTests` on vanilla `@GameTest`, `port/excludes.txt` empty, **Fabric 263/263 green**; the NeoForge half is blocked on NeoForge 21.1 needing `@GameTestHolder`/`@PrefixGameTestTemplate` on the declaring class, with three options raised for the owner (track A, A6.1 as built). Hook stage `unit` (not raised to `full` until the NeoForge suite runs). Includes `26.1.2` `7839f271` (cherry-picked as `e33f5792`). |
...
|---|---|---|---|
| A6.1 | Shared `FishtasticGameTests` harness (D10) + `fishtastic:empty` structure | [~] | | **Built and Fabric-green 2026-09-25.** One shared class of 262 wrappers on vanilla `@GameTest(template = "fishtastic:empty")`, generated by `build/a61-gen-shared-harness.py` (262 in / 262 out asserted); `FishtasticTestSupport` is the per-platform mock-player seam; new `data/fishtastic/structure/empty.nbt` (8x8x8, `DataVersion 3955`, explicit tag types). Deletes the 1,416-line Fabric harness, the 702-line 26.1-only NeoForge registration and the 91-line `GameTestStructureProvider`; `port/excludes.txt` is empty. **`:fabric:runGametest` 263 tests, 0 failures.** The Six Fabric-only tests are in the shared class. **NeoForge blocked**: NF 21.1 has no `TestData`/`GameTestInstance`/`TestEnvironmentDefinition`, and its annotation-driven path needs `@GameTestHolder` + `@PrefixGameTestTemplate(false)` on the *declaring* class (a subclass cannot help - `getDeclaredMethods()`), which a class compiled for both loaders cannot carry. Fails as `No test functions were given!` then `ResourceLocationException ... minecraft:fishtasticgametests.fishtastic:empty`. Three resolutions raised (track A, A6.1 as built); recommendation: a generated NeoForge-side harness class. **Also lands A2.8.c's Sunset Postcard gametest** (`dayTimeAdvancesAtTheAppliedRate`, rate read live rather than hard-coded). |
| A6.2 | Green bar: 78 / 163+1 / 21,955 / 262 Fabric / 262 NeoForge (+ the Sunset Postcard test) / datagen clean / fishsim byte-identical | [ ] | | **Counts corrected:** the 26.1.2 harness has **262** `@GameTest` methods, not 263 (that count included a javadoc mention); the older Fabric report's 263 also carried the Fabric API's own `minecraft:always_pass`, which the shared harness no longer picks up. Fabric measures **263** today (262 shared + the Sunset Postcard test). The NeoForge task is **`:neoforge:runGametest`**, not `:neoforge:runGameTestServer`. |
