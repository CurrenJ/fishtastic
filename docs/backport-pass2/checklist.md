...
| B2.6 | Tank BE ↔ item: `setPlacedBy`, `fishtastic:copy_tank_data`, `getCloneItemStack` | [~] | B2.6 commit | Data-flow methods written and match the doc's design (`FishTankBlockEntity#writeToItem`/`applyFromItem`, `FishTankBlock#setPlacedBy`/`getCloneItemStack`, `loot/CopyTankDataFunction`), but `FishTankBlock.java`/`FishTankBlockEntity.java` stay on `port/excludes.txt` (full-file porting — menu/registration/interaction graph — is out of this slice's scope) and the new loot function is excluded too (it names the real `FishTankBlockEntity`, not its bare portstub). Loot table JSON edit and registration wiring not done — see track-b-1.20.1.md "B2.6 as built". |
