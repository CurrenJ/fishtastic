...
**A5.4 (checkpoint 5).**
- **Glint.** The ancestor's two mixins come back with their shapes unchanged. `mixin/ItemRendererMixin` injects at HEAD/RETURN of `ItemRenderer.render` to hold the stack being drawn in a thread-local, and cancellably at HEAD of `getFoilBuffer`, `getFoilBufferDirect` and `getCompassFoilBuffer` (MC `ItemRenderer.java:176,182`) to hand back the effect's glint buffer instead of vanilla's. `RenderBuffersMixin` registers each custom glint render type as a fixed buffer on `RenderType.glint()` (the ancestor's `put` at HEAD). **Gotcha:** the local holding `RenderBuffers.getFixedBuffers()` must be declared `Map`, not `SequencedMap` — naming that type in a local emits a class constant `:common:java17ApiGuard` bans, and its allow-list covers only descriptors. Any other `Sequenced*` local hits the same wall.
- **`FishtasticGlintState` shrinks to two thread-locals** (`SILHOUETTE_REQUESTED`, `BLACK_OUTLINE_REQUESTED`). 26.1's quads-identity map, `FishtasticItemStackRenderState`, `ItemModelResolverMixin`, `ItemStackRenderStateMixin`, `ItemFeatureRendererMixin` and `RenderTypeMixin` are all deleted: the outline's render type comes from the atlas, and the UBO `RenderTypeMixin` bound is replaced by uniforms set per draw.
- **Outline atlas** (`FishtasticItemOutlineAtlas`, the spike class with its findings applied): two `TextureTarget`s (the mask with depth, the outline without), padded slots, LRU eviction, and `GameRendererMixin`'s HEAD bake point, which restores the projection and model-view matrices and rebinds the main target afterwards. A slot is baked by a direct `ItemRenderer.render(..., GUI, ...)` into a shared `MultiBufferSource` + `endBatch()`, then composed through `outline_bake`/`outline_bake_legendary` with the slot's `FishtasticOutlineStyle` (port-only: 26.1's params UBO, now plain uniforms set before the draw). An animated (pinwheel) slot recomposes every frame and is never cached. **F3 applied:** both atlas textures stay LINEAR; the world render type forces NEAREST through its own texture state shard, so nothing toggles filters per draw. **F5** was left as the design allowed ("harmless either way"): nothing filters foil out of the mask bake.
- **GUI outline** (`FishtasticGuiOutlineRenderer`): the hook is `mixin/GuiGraphicsMixin`, a HEAD inject on `GuiGraphics.renderItem(LivingEntity, Level, ItemStack, int, int, int, int)`, which flushes the GUI buffer and blits the slot's ring — a quad 1.5x the item (a 16x16 item gets 24x24), offset by 4 GUI pixels — at `ITEM_Z`. It also draws the black gear ring and, when `SILHOUETTE_REQUESTED` is set, blits the mask slot and returns true so the item underneath isn't drawn. 26.1 samples vanilla's GUI item atlas here; 1.21.1 renders GUI items straight to the screen and has none, so the **same** Fishtastic atlas feeds the world outline, the GUI outline and the silhouette. The slot is sampled LINEAR: the atlas holds 4 texels per GUI pixel, and NEAREST minification at GUI scales below 4 drops the thinnest rings.
- **World outline** (`FishtasticWorldOutlineRenderer`): an inject in `ItemRenderer.render` right after the model-space `translate(-0.5, -0.5, -0.5)`, where a flat item's sprite is exactly `[0,1]²` at `z = 0.5`. One expanded quad through `FishtasticRenderTypes.ITEM_OUTLINE` for GROUND, FIXED and FIRST_PERSON_*, skipping `isGui3d()` and `isCustomRenderer()`. It covers dropped items, item frames, tank fish (A5.1) and the held item without 26.1's three renderer mixins. **F4 applied:** the output state is `ITEM_ENTITY_TARGET` (a no-op outside Fabulous), so the outline follows the item into the item-entity target in Fabulous. Nothing is registered with Iris because there is nothing to register — see A5.7.
- **GUI shader effects.** `FishtasticSilhouetteEffect` and `FishtasticTextureOutlineEffect` each set their program with `RenderSystem.setShader(() -> shader)` and draw with `BufferUploader.drawWithShader` inside gelatin's `apply(IRenderContext)`, over the two GUI programs (`gui_item_silhouette`, `gui_texture_outline`, sharing `gui_effect.vsh`) loaded as ordinary `ShaderInstance`s. The black gear ring needs **no program of its own**: it is `FishtasticOutlineStyle.BLACK`, baked into the same atlas and drawn by the GUI outline renderer like a quality ring, because that shader is already fully parameterized. `FishtasticOutlineUboRegistry` is deleted. `ItemEffectRenderData` carries only the per-effect glint `RenderType`, keyed by effect id, rebuilt on `ItemEffectManager` reload and invalidating the atlas.
- **Deviation from the design:** `outline_debug_uv` is **not** ported (the design recommended porting it as `outline_debug_uv.json`). No program and no JSON: the `ItemEffect` codec still parses the field, nothing consumes it, and no shipped effect sets it, so the flag is inert on 1.21.1.
- **Verified on both loaders** (`outline`, `fabulous`, `guiscale`, `items` scenes): quality glint on the held item and in the hotbar; static and animated legendary GUI rings in the hotbar and inside a container; the world outline on a ground item and in an item frame; GUI scales 1, 2 and 4; Fabulous active and restored. Iris + Complementary is G1's.

**A5.7 (checkpoint 6).**
- **`IrisCompat` is back as a no-op with the same public method shape** (`assignPipeline(RenderType, String, String)`), so `FishtasticRenderTypes`' call sites read the same on both branches. Iris 1.8 recognises a render type by the vanilla shader it names — `rendertype_entity_translucent`, which shaderpacks already override — so `TANK_WATER_FILL` and `ITEM_OUTLINE` draw correctly under Complementary with nothing registered (spike F1/F4). The class comment says as much, and `[1.20.1]` is the same no-op.
- **`LevelRendererMixin` is deleted** (it was already an empty placeholder on 26.1.2) and its `fishtastic.mixins.json` entry with it.
- **The cosmetic-capture gizmos have a world-render home.** `CosmeticCaptureClientState` keeps 26.1's colours and geometry but drops `tickGizmos()`/`Gizmos` for `render(PoseStack, MultiBufferSource, Vec3 camera)`: the corner selection is stroked through the new port-only `FishtasticRenderTypes.GIZMO_LINES` (`POSITION_COLOR_NORMAL`, `LINES`, a `LineStateShard(OptionalDouble.of(2.0))` so the 2 px width doesn't leak into `RenderSystem` state, `ITEM_ENTITY_TARGET` — a no-op outside Fabulous), and the anchor box is filled with `DebugRenderer.renderFilledBox` and then stroked. Both loaders call it from their world-render hook, which hands over a **world-space** pose stack (so the body translates by `-camera`): Fabric `WorldRenderEvents.AFTER_ENTITIES`, NeoForge `RenderLevelStageEvent.Stage.AFTER_ENTITIES`. The self-test drives it through the real command path — `capture start`, three picks, a sync packet, `capture cancel` — so the packet-handler registration is exercised too.
- **HUD layers above vanilla's toasts** (owner decision, 2026-09-24). `client/FishtasticHudLayers` (port-only) holds the three layers in their 26.1 registration order, and `GameRendererMixin` injects *after* the `"toasts"` section (the `ToastComponent.render` call) to draw them while no screen is open. `GameRenderer.render`'s order is HUD (1081) -> screen (~1098) -> toasts (1140), so with a screen open the loaders' HUD hooks keep drawing them in the HUD pass, under the screen — drawing them after the toasts then would put them over the screen too. The two paths are disjoint by construction: the mixin's guard is the negation of `FishtasticHudLayers.drawnInHudPass()`. The self-test asserts that rather than eyeballing it: `hud.drawnAfterToasts` (a rising `FishtasticHudLayers.toastPassFrames` while no screen is open) and `hud.hudPassWithScreen` (the same counter flat while one is). The counter sits on the toast-pass entry point `FishtasticHudLayers.renderAfterToasts` and not on `render`, which the loaders' hooks also call: counting there counted **both** paths on Fabric (whose single `HudRenderCallback` calls `render`) and only the toast pass on NeoForge (whose HUD event is split pre/post and calls the two layer methods directly), and the first run of the strengthened scene failed on Fabric for exactly that reason — `toastPassFrames=185` with a screen open, which is the HUD pass going through `render`, not a leak.
- **Correction to A4, and to the premise of that decision.** A4 recorded that 26.1.2 draws the quest notification *above* vanilla's toasts, its HUD layers resolving against `minecraft:toast` through `HudElementRegistry`. 26.1.2 does no such thing: its `GameRenderer` extracts the HUD at line 508 and the toasts at 562, so the toasts cover the HUD there too, and the behavioural difference A4 reported is not one. The owner was told; the 1.21.1-only mixin stays, so the branches remain deliberately different here (`docs/backport-pass2/track-a-1.21.1.md`, A4's HUD note, carries the same correction).
- **Found while verifying the banner** (report only, not fixed): `/fishtastic testquestnotify` calls `QuestProgressNotificationManager.getInstance().enqueue(...)` — a client-side singleton — from the **server** thread. In single-player that mutates `active`/`pending` while the client thread's `tick()` iterates them, so the event is sometimes lost (the first NeoForge pass photographed the toast with no banner). Nothing version-sensitive: the command is the same on 26.1.2. The self-test enqueues on the client thread instead — the same call with the same event.
- **Verified on both loaders** (`hud`, `gizmos` scenes): with no screen open a quest banner and a vanilla `SystemToast` share the top-right corner **with the banner drawn over the toast** (the tutorial overlay, centred, is drawn by that same toast-pass hook — nothing else can draw it while no screen is open); with a `ChatScreen` open the toast-pass hook stays out of the frame (its counter flat at 0) and the layers are drawn by the loaders' hooks instead — and the same banner then reads two ways there, because the HUD pass (1081) is *under* the screen and the toasts (1140) are *over* it. `banner_with_screen_toast` is the toast with the banner behind it, i.e. an empty corner — the case the toast-pass hook exists to avoid, and the look 26.1.2 has always had; `banner_with_screen` clears the toasts (a step earlier than the shot — a capture is of the frame before the step that asked for it ran, so same-step clear-and-shoot photographed the toast twice) and shows the banner itself, under a transparent `ChatScreen` at its right margin. Reading that took a dump of the manager's state, not an eye: while the screen was open the run logged `active=1 pending=0 stagger=0 mult=1.00 level=true guiW=534 {mastery/bluegill_novice HOLD delay=0 x=455.0 w=69.0}`, and `x=455` is `534 - 69 - 10`, the right margin — a live banner in its slot, drawn and outranked, not missing. (The first version of this half enqueued a **fresh** banner *after* the screen opened and photographed that: same picture, and the burst it used was a no-op besides, since `enqueue` updates an already-active notification of the same quest in place instead of sliding a new one in.) The wand gizmos draw the yellow corner stroke and the magenta anchor fill + stroke, and both disappear when the session is cancelled.
