...
> **Baseline (A0.3, moved forward 2026-09-24):** branch `26.1.2` @ `51a8d367` (Fishtastic 2.0.1 + seams S3, S1, S5, S6, S6c, plus the datagen and NeoForge-gametest fixes, MC 26.1.2). It was first frozen at `298279e1` (S3 + S1). Pass 1 surveyed `7076aeb6`, the parent of the seams.
...
- **A0.3** ~~Freeze the feature baseline: the `26.1.2` commit that lands S1 and S3 (D5).~~ Done 2026-09-24: first **`298279e1`** (S1 + S3), then moved forward the same day to **`63c8716a`** after S5/S6 (D9) and two gate fixes landed, and then to **`51a8d367`** after S6c (`util/Ids`). `port/1.21.1` is rebased onto it. Forward-port ranges start from this commit ("ported through `51a8d367`").
...
- **S6: `FishMoonPhase` and a permission helper** *(pass 2, N2, D9)*.
  **Done 2026-09-24** (`c321593a`). `data/FishMoonPhase` copies 26.1's `MoonPhase`: the same constant names, serialized names (`full_moon` … `waxing_gibbous`) and indices, with a `StringRepresentable` `CODEC`, `fromIndex(int)`, `fromVanilla(MoonPhase)`, and `at(Level, BlockPos)`. `at` is the one method whose body differs per branch: 1.21.1 and 1.20.1 return `fromIndex(level.getMoonPhase())`. `FishProfile.MoonWeight`, `computeEnvironmentMultiplier`, `FishtasticFishItem`, `FishingMinigameManager` and the encyclopedia's moon row use it. The 64 fish-profile JSONs are unchanged, and the names were checked against vanilla's source. `command/FishtasticPermissions.gamemaster()` returns `Predicate<CommandSourceStack>`. On 26.1.2 that's vanilla's own `Commands.hasPermission(LEVEL_GAMEMASTERS)` object (vanilla's `ArgumentUtils` checks that type, so it isn't wrapped in a lambda). All 21 `requires` sites use it: the 20 commands plus `mcp/McpBridgeCommand`, which A1.3 deletes.
  **Gate fixes that landed with the seams.** `f2085c7b`: the book recipes had been hand-edited in `92194f2f` without updating `FishtasticRecipeProvider`, so `runDatagen` reverted them. The provider now generates them, and `runDatagen` leaves no content changes. `63c8716a`: `:neoforge:runGametest` had been running only vanilla's 1 test. Loom's default `main` mod group shadowed the `fishtastic` group that carries testmod, so `NeoForgeGameTestRegistration` never loaded. Naming our group `main` fixed it: 257 tests run (256 Fishtastic + 1 vanilla).
  **Gate at `63c8716a`:** `:common:test` 78, fishsim 163 + 1 skipped, tank-shape-gen 21,955, Fabric gametests 263, NeoForge gametests 257, `runDatagen` content-clean, fish-profile JSONs byte-identical.
- **S6c: `util/Ids` for resource id construction** *(pass 2, B5.1)*.
  **Done 2026-09-24** (`c8129cf3`, guard `51a8d367`). 26.1.2 builds ids with `Identifier`'s static factories, 1.21.1 has the same factories on `ResourceLocation`, and 1.20.1 only has `new ResourceLocation(...)`. `grill24.fishtastic.util.Ids` wraps the four factories the tree uses: `of(ns, path)` (`fromNamespaceAndPath`, 77 calls), `withDefaultNamespace` (45), `parse` (8) and `tryParse` (5). All 135 calls in 43 files (common, fabric, neoforge, tests, testmods and datagen) go through it, so 1.20.1's B5.1 rewrite shrinks to the four `Ids` bodies. Nine files lost their now-unused `Identifier` import. No behaviour change: datagen output is identical and the gametests pass.
  ...
  **Caveat.** `Ids` hides construction, not the type. A2.1's `Identifier` → `ResourceLocation` type rename still happens on 1.21.1 (100 files at `51a8d367`; 109 before S6c). It's type-only there: `Ids`' bodies keep the same factory names on `ResourceLocation`.
  **Gate at `51a8d367`** (after S6c): the same counts (78 / 163 + 1 / 21,955 / 263 Fabric / 257 NeoForge), datagen output content-identical, and both guards clean on common, fabric and neoforge.
...
| **D9** | *(pass 2)* Land seams S5 (Java 17 calls) and S6 (`FishMoonPhase`, permission helper) on `26.1.2` before track A? | **Decided 2026-09-24: yes.** S5 and S6 land on `26.1.2` before A1; the baseline marker advances past `298279e1`. **Done:** `552fed58`..`c321593a`, plus S6c (`c8129cf3`, `51a8d367`); baseline now `51a8d367`. |
