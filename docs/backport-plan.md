...
> **Status:** Pass 1, the broad-strokes outline (2026-09-24). A-SPIKE passed. Decisions D1–D11 all settled (§10; D8–D11 on 2026-09-24). Seams S3, S1, S5, S6 and S6c landed on `26.1.2` (A0.3 baseline below).
...
> **Baseline (A0.3, moved forward 2026-09-24):** branch `26.1.2` @ `33986055` (Fishtastic 2.0.1 + seams S3, S1, S5, S6, S6c, plus the datagen and NeoForge-gametest fixes, MC 26.1.2). It was first frozen at `3b8427e4` (S3 + S1). Pass 1 surveyed `7076aeb6`, the parent of the seams.
...
- **A0.3** ~~Freeze the feature baseline: the `26.1.2` commit that lands S1 and S3 (D5).~~ Done 2026-09-24: first **`3b8427e4`** (S1 + S3), then moved forward the same day to **`f096fc8d`** after S5/S6 (D9) and two gate fixes landed, and then to **`33986055`** after S6c (`util/Ids`). `port/1.21.1` is rebased onto it. Forward-port ranges start from this commit ("ported through `33986055`").
...
> The full report, evidence and findings are in `docs/spike-1.21.1-rendering.md` on branch `spike/1.21.1-rendering` (commit `362b5255`).
...
- **S1: Item data accessor.** Route every `stack.get/set/has(FishtasticDataComponents.X)` and the vanilla `DataComponents.*` reads the mod depends on through the existing helpers (`FishQualityHelper`, `ItemSizeHelper`, …) or a thin `FishtasticItemData` facade. On 1.20.1, only that facade gets an NBT implementation.
  **Done 2026-09-24** (`a285e76f`..`3b8427e4`, 4 commits). `common/.../FishtasticItemData` is now the only place that calls `ItemStack#get/getOrDefault/has/set/remove`, `applyComponents` or `isSameItemSameComponents` (154 call sites in 47 files, including the shared gametests and the platform tank item models). Its shape:
...
- **S3: Pin `fishsim` and `tools:tank-shape-gen` to `--release 17`** and replace the handful of Java 21 library calls there. These modules ship in the mod jar and feed datagen. Kept at 17, they are **one shared source across all three versions**: no branch-specific fishsim, and fishsim fixes merge cleanly everywhere.
  **Done 2026-09-24** (`2b181f3c`). `options.release = 17` is set on every JavaCompile task (main and test) in both modules. The toolchains are unchanged (25 and 21), and the class files are major version 61. The compiler found only 8 Java 21 calls, all `List.getFirst()/getLast()`. Behaviour is byte-identical: same test counts (fishsim 163 + 1 skipped, tank-shape-gen 21,955 including the STANDARD gate), and a headless fishsim export (L, 12 fish, seed 42, 3000 ticks) produces identical CSV, PNGs and GIF.
- **S5: Java 17 library calls** *(pass 2, D9)*. Replace the Java 21-only calls in common, fabric and neoforge (including the testmods), so 1.20.1 compiles the same sources.
  **Done 2026-09-24** (`cf643423`, guard `afa6d4dc`). 19 `List#getFirst()` → `get(0)` (11 files, 3 of them gametests), 7 `Math.clamp` → `Mth.clamp` (the int/float/double overloads exist on all three versions and agree for every input these sites pass), and `RenderBuffersMixin`'s `fixedBuffers` local declared as `Map`. Pass 2's N7 count (~45) overstated it: all 13 `reversed()` hits are `Comparator#reversed` (Java 8), the `addFirst`/`addLast`/`removeFirst` hits are on `Deque`s or are MC/Fabric API methods, and `McpTokenAuth`'s `getFirst` is `com.sun.net.httpserver.Headers`. `BufferSourceAccessor` keeps `SequencedMap`: Mixin matches an `@Accessor`'s target by the field's exact descriptor, and the field is a `SequencedMap` on 26.1.2 and 1.21.1 (1.20.1's render rewrite changes it anyway).
  **Guard.** `--release 17` can't be used on these modules (they compile against MC's Java 25 class files), so `gradle/backport-guards.gradle` (named `java17-api-guard.gradle` until S6c) adds a `java17ApiGuard` task to every Loom subproject. It reads the constant pool of every compiled class in every source set and fails on the sequenced-collection methods owned by a non-`Deque` `java.util` type, `reversed()` on anything but `Comparator`, `Math.clamp`, and the `Sequenced*` types. It checks bytecode, not sources, because javac records the receiver's static type as the method owner, so `List#reversed` and `Comparator#reversed` are distinguishable there and not in a grep. It runs before `:common:test` and each `runGametest` (so the pre-commit hook covers it) and in `check`. It has no dependencies, so it runs on the port branches' JDK 21/17 Gradle too. Only `BufferSourceAccessor` and `RenderBuffersMixin` are allowlisted, for the descriptor above.
- **S6: `FishMoonPhase` and a permission helper** *(pass 2, N2, D9)*.
  **Done 2026-09-24** (`616b6566`). `data/FishMoonPhase` copies 26.1's `MoonPhase`: the same constant names, serialized names (`full_moon` … `waxing_gibbous`) and indices, with a `StringRepresentable` `CODEC`, `fromIndex(int)`, `fromVanilla(MoonPhase)`, and `at(Level, BlockPos)`. `at` is the one method whose body differs per branch: 1.21.1 and 1.20.1 return `fromIndex(level.getMoonPhase())`. `FishProfile.MoonWeight`, `computeEnvironmentMultiplier`, `FishtasticFishItem`, `FishingMinigameManager` and the encyclopedia's moon row use it. The 64 fish-profile JSONs are unchanged, and the names were checked against vanilla's source. `command/FishtasticPermissions.gamemaster()` returns `Predicate<CommandSourceStack>`. On 26.1.2 that's vanilla's own `Commands.hasPermission(LEVEL_GAMEMASTERS)` object (vanilla's `ArgumentUtils` checks that type, so it isn't wrapped in a lambda). All 21 `requires` sites use it: the 20 commands plus `mcp/McpBridgeCommand`, which A1.3 deletes.
  **Gate fixes that landed with the seams.** `f896635b`: the book recipes had been hand-edited in `92194f2f` without updating `FishtasticRecipeProvider`, so `runDatagen` reverted them. The provider now generates them, and `runDatagen` leaves no content changes. `f096fc8d`: `:neoforge:runGametest` had been running only vanilla's 1 test. Loom's default `main` mod group shadowed the `fishtastic` group that carries testmod, so `NeoForgeGameTestRegistration` never loaded. Naming our group `main` fixed it: 257 tests run (256 Fishtastic + 1 vanilla).
  **Gate at `f096fc8d`:** `:common:test` 78, fishsim 163 + 1 skipped, tank-shape-gen 21,955, Fabric gametests 263, NeoForge gametests 257, `runDatagen` content-clean, fish-profile JSONs byte-identical.
- **S6c: `util/Ids` for resource id construction** *(pass 2, B5.1)*.
  **Done 2026-09-24** (`24203a87`, guard `33986055`). 26.1.2 builds ids with `Identifier`'s static factories, 1.21.1 has the same factories on `ResourceLocation`, and 1.20.1 only has `new ResourceLocation(...)`. `grill24.fishtastic.util.Ids` wraps the four factories the tree uses: `of(ns, path)` (`fromNamespaceAndPath`, 77 calls), `withDefaultNamespace` (45), `parse` (8) and `tryParse` (5). All 135 calls in 43 files (common, fabric, neoforge, tests, testmods and datagen) go through it, so 1.20.1's B5.1 rewrite shrinks to the four `Ids` bodies. Nine files lost their now-unused `Identifier` import. No behaviour change: datagen output is identical and the gametests pass.
  **Guard.** `idConstructionGuard` fails on any call to an `Identifier`/`ResourceLocation` factory or constructor outside `Ids`. It reuses the S5 guard's constant-pool reader, so the script became `gradle/backport-guards.gradle`: a shared `BackportGuard` base with two checks, `java17ApiGuard` (rules unchanged) and `idConstructionGuard`. Both run before `:common:test` and each `runGametest`, and in `check`. Checked against a probe class: all four factories and every S5 rule were flagged; `Ids` itself, `Deque#addLast` and `Comparator#reversed` were not.
  **Caveat.** `Ids` hides construction, not the type. A2.1's `Identifier` → `ResourceLocation` type rename still happens on 1.21.1 (100 files at `33986055`; 109 before S6c). It's type-only there: `Ids`' bodies keep the same factory names on `ResourceLocation`.
  **Gate at `33986055`** (after S6c): the same counts (78 / 163 + 1 / 21,955 / 263 Fabric / 257 NeoForge), datagen output content-identical, and both guards clean on common, fabric and neoforge.
...
- **B1.2** Java 17 source audit. *(Now a no-op: S5 did it on `26.1.2`, and `java17ApiGuard` keeps it done. On 1.20.1 the Java 17 toolchain is the final check.)* S3 is done, so `fishsim` and `tank-shape-gen` need nothing (they are `--release 17` on every branch).
...
- **B5.1** `ResourceLocation.fromNamespaceAndPath` → `new ResourceLocation(...)`, in the four `util/Ids` bodies only *(S6c)*. `SavedData.Factory` → `computeIfAbsent(load, create, name)`. BE NBT without `HolderLookup.Provider`.
...
| **D9** | *(pass 2)* Land seams S5 (Java 17 calls) and S6 (`FishMoonPhase`, permission helper) on `26.1.2` before track A? | **Decided 2026-09-24: yes.** S5 and S6 land on `26.1.2` before A1; the baseline marker advances past `3b8427e4`. **Done:** `cf643423`..`616b6566`, plus S6c (`24203a87`, `33986055`); baseline now `33986055`. |
