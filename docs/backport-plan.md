...
> **Status:** Pass 1, the broad-strokes outline (2026-09-24). A-SPIKE passed. Decisions D1–D11 all settled (§10; D8–D11 on 2026-09-24). Seams S3, S1, S5 and S6 landed on `26.1.2` (A0.3 baseline below).
...
> **Baseline (A0.3, moved forward 2026-09-24):** branch `26.1.2` @ `63c8716a` (Fishtastic 2.0.1 + seams S3, S1, S5, S6, plus the datagen and NeoForge-gametest fixes, MC 26.1.2). It was first frozen at `298279e1` (S3 + S1). Pass 1 surveyed `7076aeb6`, the parent of the seams.
...
- **A0.3** ~~Freeze the feature baseline: the `26.1.2` commit that lands S1 and S3 (D5).~~ Done 2026-09-24: first **`298279e1`** (S1 + S3), then moved forward the same day to **`63c8716a`** after S5/S6 (D9) and two gate fixes landed. `port/1.21.1` is rebased onto it. Forward-port ranges start from this commit ("ported through `63c8716a`").
...
- **S6: `FishMoonPhase` and a permission helper** *(pass 2, N2, D9)*.
  ...
  **Not done: `Ids.of(ns, path)`** (B5.1), which would remove the 1.20.1 `ResourceLocation` constructor rewrite. It's waiting on an owner decision.
...
| **D9** | *(pass 2)* Land seams S5 (Java 17 calls) and S6 (`FishMoonPhase`, permission helper) on `26.1.2` before track A? | **Decided 2026-09-24: yes.** S5 and S6 land on `26.1.2` before A1; the baseline marker advances past `298279e1`. **Done:** `552fed58`..`c321593a`, baseline now `63c8716a`. |
