...
public class QuestTracker {
    ...
    public static void onCatch(MinecraftServer server, ServerPlayer player,
        ...
        if (caughtStack.isEmpty() || !caughtStack.is(ItemTags.FISHES)) return;
        ...
        PlayerQuestState state = catchData.getOrCreateQuestState(player.getUUID());
        ...
        for (Map.Entry<ResourceKey<Quest>, Quest> entry : questRegistry.entrySet()) {
            ...
            if (matchesObjective(quest.objective(), caughtStack, biome, timeOfDay, weather)) {
                state.incrementCount(questKey, quest);
            }
        }

        catchData.setDirty();
        QuestSyncPacket.sendToPlayer(player, catchData);
    }
