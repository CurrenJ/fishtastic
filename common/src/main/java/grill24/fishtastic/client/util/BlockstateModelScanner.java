...
/**
 * Scans blockstate JSONs from the resource manager and builds a redirect map from each block's
    ...
    public static Map<Identifier, Identifier> buildRedirectMap(ResourceManager manager) {
        ...
        for (Map.Entry<Identifier, Resource> entry : blockstates.entrySet()) {
            ...
            try (Reader reader = entry.getValue().openAsReader()) {
                ...
                Fishtastic.LOGGER.debug("[BlockstateModelScanner] Skipping {}: {}", entry.getKey(), e.getMessage());
            }
        }

        Fishtastic.LOGGER.info("[BlockstateModelScanner] Built redirect map with {} entries.", redirects.size());
        return redirects;
    }
